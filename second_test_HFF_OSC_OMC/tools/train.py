...
def build_opt_lr(model, current_epoch=0):
    ...
    for i in range(1):
        for name, value in model.MSCABModelArr[i].named_parameters():
            value.requires_grad = False
        for m in model.MSCABModelArr[i].modules():
            if isinstance(m, nn.BatchNorm2d):
                m.eval()
    for name, value in model.MSCABModelArr[1].named_parameters():
            value.requires_grad = True
    for name, value in model.MSCABModelArr[2].named_parameters():
            value.requires_grad = True
    for name, value in model.MSCABModelArr[3].named_parameters():
            value.requires_grad = True
    for name, value in model.MSCABModelArr[4].named_parameters():
            value.requires_grad = True
    for param in model.head.parameters():
        param.requires_grad = True
    for param in model.chanModel.parameters():
        param.requires_grad = True
    ...
    trainable_params += [{'params': model.head.parameters(),
                          'lr': 0.0005}] 
    trainable_params += [{'params': model.MSCABModelArr[1].parameters(),
                          'lr': 0.0005}] 
    trainable_params += [{'params': model.MSCABModelArr[2].parameters(),
                          'lr': 0.0005}] 
    trainable_params += [{'params': model.MSCABModelArr[3].parameters(),
                          'lr': 0.0005}] 
    trainable_params += [{'params': model.MSCABModelArr[4].parameters(),
                          'lr': 0.0005}] 
            ...
            if (idx+1) % 50 == 0:
                torch.save(
                        {'epoch': epoch,
                         'state_dict': model.module.state_dict(),
                         'optimizer': optimizer.state_dict()},
                        cfg.TRAIN.SNAPSHOT_DIR+'/checkpoint_e%d_idx%d.pth' % (epoch+1,idx+1))
        end = time.time()
