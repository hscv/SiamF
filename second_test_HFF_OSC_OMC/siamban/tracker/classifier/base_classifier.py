...
class BaseClassifier(object):
    ...
    def __init__(self, model, abundanceModel):
        self.model = model
        self.AbundanceModel = abundanceModel

    def initialize(self, image, state, endLib, endNum):
        ...
        if cfg.TRACK.ABUNDANCE_UPDATE:
            train_x, init_abundance = self.generate_init_samples(image)
        else:
        ...
        tt_b,_,_,_ = init_y[0].size()

        # Init memory
        if cfg.TRACK.ABUNDANCE_UPDATE:
            self.init_memory(train_x, init_abundance)
        else:
            self.init_memory(train_x, None)
        ...
        if cfg.TRACK.ABUNDANCE_UPDATE:
            self.init_abundance_optimization(init_abundance, init_y)


    def init_abundance_optimization(self,init_abundance, init_y):
        label = init_y[0]
        b,c,w,h = init_abundance.size()
        abModel = self.AbundanceModel.cuda()
        criterion = nn.MSELoss().cuda()
        optimizer = optim.SGD(abModel.parameters(), lr=0.01, momentum=0.9, weight_decay=0.01)
        num_epochs = 100
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(b//8):
                optimizer.zero_grad()
                train_data = init_abundance[rand_index[i*8:(i+1)*8],:,:,:]
                train_data = train_data.float().cuda()
                label_data = label[rand_index[i*8:(i+1)*8],:,:,:]
                label_data = label_data.float().cuda()

                train_data = train_data.unsqueeze(1)
                outputs = abModel(train_data)
        ...
        for _ in range(1):
            ...
            for i in range(b//8):
                ...
                for kk in range(batch_size):
                    zhixintu = getZhiXinPic(label_data[kk,0,:,:].detach().cpu().numpy())
                    saveName = './init_y_zhixintu/batch_%02d_label_%02d.jpg' % (i, kk)
                    cv2.imwrite(saveName, zhixintu)

                for kk in range(batch_size):
                    zhixintu = getZhiXinPic(outputs[kk,0,:,:].detach().cpu().numpy())
                    saveName = './init_y_zhixintu/batch_%02d_out_%02d.jpg' % (i, kk)
                    cv2.imwrite(saveName, zhixintu)
    ...
    def updateAbundanceModel(self, epoch_num):
        label = self.y[0]

        abundance_samples = self.training_abundance_samples
        b,c,w,h = abundance_samples.size()
        abModel = self.AbundanceModel.cuda()
        criterion = nn.MSELoss().cuda()
        optimizer = optim.SGD(abModel.parameters(), lr=1e-3, momentum=0.9, weight_decay=0.01)
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(b//8):
                optimizer.zero_grad()
                train_data = abundance_samples[rand_index[i*8:(i+1)*8],:,:,:]
                train_data = train_data.float().cuda()
                label_data = label[rand_index[i*8:(i+1)*8],:,:,:]
                label_data = label_data.float().cuda()
    ...
    def init_optimization(self, train_x, init_y):
        ...
        if self.update_projection_matrix:
            ...
            # Initialize optimizer ------- False
            plot_show = False and cfg.TRACK.DEBUG_CLASS and cfg.TRACK.VISUALIZE_CLASS
    ...
    def update(self, state, scale_z, flag, abundance_x):
        ...
        if update_flag:
            ...
            # Update memory
            self.update_memory(train_x, train_y, abundance_x, learning_rate)

        # Train filter
        if hard_negative:
            self.filter_optimizer.run(cfg.TRACK.HARD_NEGATIVE_CG_ITER)
            if cfg.TRACK.ABUNDANCE_UPDATE:
                self.updateAbundanceModel(cfg.TRACK.HARD_NEGATIVE_CG_ITER)
        elif (self.frame_num-1) % cfg.TRACK.TRAIN_SKIPPING == 0:
            self.filter_optimizer.run(cfg.TRACK.CG_ITER)
            if cfg.TRACK.ABUNDANCE_UPDATE:
                self.updateAbundanceModel(cfg.TRACK.CG_ITER)
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        init_samples,ims_augmented = self.extract_transformed_samples(im, self.transforms)
        b,c,w,h = ims_augmented.size()
        ims_augmented = ims_augmented.view(b,c,-1).permute(0,2,1).view(b,w,h,c)
        ims_augmented = ims_augmented.numpy()

        if cfg.TRACK.ABUNDANCE_UPDATE:
            init_abundance = []
            for i in range(len(ims_augmented)):
                img_tt = cv2.resize(ims_augmented[i,:,:,:],(62,62))
                init_abundance.append(getAbundance(img_tt/img_tt.max(), self.endLib, self.endNum)) 
            init_abundance = np.array(init_abundance)
            init_abundance = torch.from_numpy(init_abundance)

            if cfg.CUDA:
                init_abundance = init_abundance.cuda()
                init_abundance = init_abundance.permute(0,3,1,2)
        ...
        for i, use_aug in enumerate(TensorList([cfg.TRACK.USE_AUGMENTATION])):
            if not use_aug:
                ...
                if cfg.TRACK.ABUNDANCE_UPDATE:
                    init_abundance[i] = init_abundance[i][0:1, ...]
        # Add dropout samples
        if cfg.TRACK.AUGMENTATION_DROUPOUT:
            ...
            if use_aug and cfg.TRACK.ABUNDANCE_UPDATE:
                init_abundance = torch.cat([init_abundance, F.dropout2d(init_abundance[i].expand(num,-1,-1,-1), p=prob, training=True)])

        if cfg.TRACK.ABUNDANCE_UPDATE:
            return init_samples, init_abundance
        else:
            return init_samples
    ...
    def init_memory(self, train_x, init_abundance):
        ...
        if cfg.TRACK.ABUNDANCE_UPDATE:
            self.init_abundance_training_samples = init_abundance
        ...
        if cfg.TRACK.ABUNDANCE_UPDATE:
            _,c,w,h = init_abundance.size()
            self.training_abundance_samples = init_abundance[0].new_zeros(cfg.TRACK.SAMPLE_MEMORY_SIZE, c, w, h)

    def update_memory(self, sample_x: TensorList, sample_y: TensorList, abundance_x, learning_rate = None):
        ...
        if cfg.TRACK.ABUNDANCE_UPDATE:
            for dd in replace_ind:
                self.training_abundance_samples[dd,...] = abundance_x 
