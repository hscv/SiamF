...
from siamban.utils.imagePreDeal import ImagePreDeal_FC,ImagePreDeal_HSI
...
class SubDataset(object):
    ...
    def get_image_anno(self, video, track, frame):
        ...
        image_path = os.path.join(self.root, lastFileName+'.png')
        image_anno = self.labels[video][track][frame]
        return image_path, image_anno
    ...
    def get_positive_pair(self, index):
        ...
        video_name_pure = video_name.split('/')[1]
        cur_video_frame_number = self.video_frame_number[video_name_pure]
        template_frame = np.random.randint(0, cur_video_frame_number) 
        search_frame = np.random.randint(0, cur_video_frame_number) 
        template_video_name = '%s%04d' % (video_name[:-4],template_frame+1)
        search_video_name = '%s%04d' % (video_name[:-4],search_frame+1)
        return self.get_image_anno(template_video_name, track, 0), \
            self.get_image_anno(search_video_name, track, 0)
...
class BANDataset(Dataset):
    def __init__(self,):
        ...
        start = 0
        self.num = 0
        sub_dataset = self.readAllDataset('./trainHSI_NJUST.json', \
                                     '/data/lizf/HOT/dataset/train/', start)
        sub_dataset.log()
    ...
    def __getitem__(self, index):
        ...
        template_image, search_image = ImagePreDeal_HSI(template[0],search[0],template[1],search[1])
