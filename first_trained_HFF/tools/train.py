...
def build_data_loader():
    ...
    train_loader = DataLoader(train_dataset,
                              batch_size= cfg.TRAIN.BATCH_SIZE,  # 28
...
def build_opt_lr(model, current_epoch=0):
    ...
    ## MSCAB
    for name, value in model.MSCABModel_b1.named_parameters():
            value.requires_grad = True
    for name, value in model.MSCABModel_b2.named_parameters():
            value.requires_grad = True
    for name, value in model.MSCABModel_b3.named_parameters():
            value.requires_grad = True
    for name, value in model.MSCABModel_b4.named_parameters():
            value.requires_grad = True
    ...
    for m in model.chanModel.modules():
       if isinstance(m, nn.BatchNorm2d):
           m.train()
    ...
    #                      'lr': 0.000005}] 
    trainable_params += [{'params': model.MSCABModel_b1.parameters(),
                          'lr': 0.0005}] 
    trainable_params += [{'params': model.MSCABModel_b2.parameters(),
                          'lr': 0.0005}] 
    trainable_params += [{'params': model.MSCABModel_b3.parameters(),
                          'lr': 0.0005}] 
    trainable_params += [{'params': model.MSCABModel_b4.parameters(),
                          'lr': 0.0005}] 
